...
- **`classes/task/validate_all_courses.php`** — Tarea programada que valida todos los cursos de las categorías permitidas. Se ejecuta diariamente a las 2 AM.
...
- **`report.php`** — Página de reporte admin con pestañas (abiertas/todas), filtro por categoría, estadísticas (total issues, open, aulas validadas, % cumplimiento), gráficos Chart.js (barras por validación, donut de cumplimiento), tabla paginada y exportación CSV/Excel. Usa SQL diferenciado para PostgreSQL y MySQL.
